...
pro.connect = function(tracer, cb) {
  ...
  var self = this;
  this.socket = client.connect("http://" + this.host + ':' + this.port, {forceNew: true, 'reconnection': false});
  ...
  this.socket.on('connect_timeout', function(err) {
    logger.error('rpc socket is connect timeout, remote server host: %s, port: %s', self.host, self.port);
