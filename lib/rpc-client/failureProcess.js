var logger = require('pome-logger').getLogger('pome-rpc', 'failprocess');
...
var failover = function(code, tracer, serverId, msg, opts, cb) {
	...
	if (!servers.length) {
		logger.error('[pome-rpc] rpc failed with all this type of servers, with serverType: %s', serverType);
