...
var logger = require('pome-logger').getLogger('pome-rpc', __filename);
...
pro.stop = function(force) {
  if(this.state !== STATE_STARTED) {
    logger.warn('[pome-rpc] client is not running now.');
    return;
  }
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if(this.state !== STATE_STARTED) {
    ...
    logger.error('[pome-rpc] client is not running now.');
  ...
  if(!mailbox) {
    ...
    if(!lazyConnect(tracer, this, serverId, this.mailboxFactory, cb)) {
      ...
      logger.error('[pome-rpc] fail to find remote server:' + serverId);
  ...
  var send = function(tracer, err, serverId, msg, opts) {
    ...
    if(!mailbox) {
      ...
      logger.error('[pome-rpc] could not find mailbox with id:' + serverId);
    ...
    mailbox.send(tracer, msg, opts, function() {
      ...
      if(!!send_err) {
        logger.error('[pome-rpc] fail to send message');
 ...
 * @param  {Function}   cb     callback function  TODO: not used
 */
pro.connect = function(tracer, serverId, cb) {
  ...
  mailbox.connect(tracer, function(err) {
    if(!!err) {
      ...
      logger.error('[pome-rpc] mailbox fail to connect to remote server: ' + serverId);
...
var doFilter = function(tracer, err, serverId, msg, opts, filters, index, operate, cb) {
  ...
  if(typeof filter === 'function') {
    filter(serverId, msg, opts, function(target, message, options) {
      index++;
      //compatible for pome filter next(err) method
...
var lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  if(!server) {
    logger.error('[pome-rpc] unknown server: %s', serverId);
    return false;
  }
  if(!online || online !== 1) {
    logger.error('[pome-rpc] server is not online: %s', serverId);
  }
...
var addToPending = function(tracer, station, serverId, args) {
  ...
  if(pending.length > station.pendingSize) {
    ...
    logger.warn('[pome-rpc] station pending too much for: %s',  serverId);
    return;
  }
  pending.push(args);
};
...
var flushPending = function(tracer, station, serverId, cb) {
  ...
  if(!mailbox) {
    ...
    logger.error('[pome-rpc] fail to flush pending messages for empty mailbox: ' + serverId);
  }
...
var errorHandler = function(tracer, station, err, serverId, msg, opts, flag, cb) {
  if(!!station.handleError) {
    ...
    logger.error('[pome-rpc] rpc filter error with serverId: %s, err: %j', serverId, err.stack);
