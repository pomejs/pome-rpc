...
pro.start = function(cb) {
  ...
  this._station.start(function(err) {
    if (err) {
      logger.error('[pome-rpc] client start fail for ' + err.stack);
      return cb(err);
    }
...
pro.rpcInvoke = function(serverId, msg, cb) {
  ...
  if (this.state !== STATE_STARTED) {
    ...
    logger.error('[pome-rpc] fail to do rpc invoke for client is not running');
    cb(new Error('[pome-rpc] fail to do rpc invoke for client is not running'));
    return;
  }
...
var proxyCB = function(client, serviceName, methodName, args, attach, isToSpecifiedServer) {
  if (client.state !== STATE_STARTED) {
    logger.error('[pome-rpc] fail to invoke rpc proxy for client is not running');
    return;
  }
  if (args.length < 2) {
    logger.error('[pome-rpc] invalid rpc invoke, arguments length less than 2, namespace: %j, serverType, %j, serviceName: %j, methodName: %j',
...
var getRouteTarget = function(client, serverType, msg, routeParam, cb) {
  if (!!client.routerType) {
    ...
    if (typeof client.router === 'function') {
      ...
      logger.error('[pome-rpc] invalid route function.');
      return;
    }
...
var rpcToSpecifiedServer = function(client, msg, serverType, serverId, cb) {
  if (typeof serverId !== 'string') {
    logger.error('[pome-rpc] serverId is not a string : %s', serverId);
    return;
  }
  if (serverId === '*') {
    var servers = client._routeContext.getServersByType(serverType);
    if (!servers) {
      logger.error('[pome-rpc] serverType %s servers not exist', serverType);
      return;
    }
